...
bool CGUIWindowFullScreen::OnAction(const CAction &action)
{
#if defined(__VIDONME_MEDIACENTER__)
#if 0
	if (VidOnMe::VDMUtils::Instance().GetPlayingMode() == VidOnMe::PM_NAVIGATION)
	{
		if (g_application.m_pPlayer != NULL && g_application.m_pPlayer->OnAction(action))
			return true;
	}
#else
	if (g_application.m_pPlayer != NULL && g_application.m_pPlayer->IsInMenu() && g_application.m_pPlayer->OnAction(action))
	{
#if 0
		if (action.GetID() == ACTION_NEXT_ITEM || action.GetID() == ACTION_PREV_ITEM)
		{
			CVDMInformationDlg* pDlg = (CVDMInformationDlg*)g_windowManager.GetWindow(VDM_WINDOW_INFORMATION);
			if (pDlg)
			{
				int nChapterCount		= g_application.m_pPlayer->GetChapterCount();
				int nCurChapter			= g_application.m_pPlayer->GetChapter();
				...
				if (g_application.m_pPlayer && nChapterCount > 1 && nCurChapter > 0 && nCurChapter <= nChapterCount)
				{
					pDlg->ShowChapterInfo(nCurChapter);
				}
				else
				{
					pDlg->ShowForbid();
  ...
  if (action.GetID() == ACTION_NEXT_ITEM || action.GetID() == ACTION_PREV_ITEM || action.GetID() == ACTION_CHANNEL_UP || action.GetID() == ACTION_CHANNEL_DOWN)
	{
		if (g_application.m_pPlayer != NULL)
		{
...
#if defined(__VIDONME_MEDIACENTER_ANDROID__) || defined(__ANDROID_ALLWINNER__) || defined(TARGET_ANDROID)
			if (g_application.IsPlaying())
			{
				//stop play and previous window
				g_application.StopPlaying();
			}
#else
			g_windowManager.PreviousWindow();
#endif //__VIDONME_MEDIACENTER_ANDROID__
	...
	case ACTION_NEXT_SUBTITLE:
		{
			...
			if (toast->IsActive())
			{
				...
				if (g_settings.m_currentVideoSettings.m_SubtitleStream != g_application.m_pPlayer->GetSubtitle())
				{
			...
			{
				g_settings.m_currentVideoSettings.m_SubtitleStream	= g_application.m_pPlayer->GetSubtitle();
				g_settings.m_currentVideoSettings.m_SubtitleOn			= g_application.m_pPlayer->GetSubtitleVisible();
			}
			...
			if (g_settings.m_currentVideoSettings.m_SubtitleOn)
			{
				...
				sub.Format("%d/%d: %s", g_settings.m_currentVideoSettings.m_SubtitleStream + 1, g_application.m_pPlayer->GetSubtitleCount(), sub);
			}
			else
				sub = g_localizeStrings.Get(1223);
      ...
      //   int num = 0;
      //   num = g_application.m_pPlayer->GetSubtitle();
      pchar = g_application.m_pPlayer->GetSubtitlePath(g_settings.m_currentVideoSettings.m_SubtitleStream);
      if(pchar)
      {
	...
	case ACTION_AUDIO_NEXT_LANGUAGE:
		{
			...
			if (toast && toast->IsActive())
			{
				...
				if (g_application.m_pPlayer->GetAudioStreamCount() > 1)
				{
					if (!m_timerAudioSwitch.IsRunning())
					{
						m_timerAudioSwitch.StartZero();
					}
					else
					{
						m_timerAudioSwitch.Reset();
					}
				}
			}
			...
			aud.Format("%d/%d: %s", g_settings.m_currentVideoSettings.m_AudioStream + 1 , g_application.m_pPlayer->GetAudioStreamCount() , aud);
	...
	case GUI_MSG_WINDOW_DEINIT:
		{
			...
			{
				pDialog = (CGUIDialog *)g_windowManager.GetWindow(VDM_WINDOW_MOVIE_OSD);
				if (pDialog) pDialog->Close(true);
				pDialog = (CGUIDialog *)g_windowManager.GetWindow(VDM_WINDOW_MOVIE_OSD_KEY);
				if (pDialog) pDialog->Close(true);
				pDialog = (CGUIDialog *)g_windowManager.GetWindow(VDM_WINDOW_MOVIE_MENU);
				if (pDialog) pDialog->Close(true);
				pDialog = (CGUIDialog *)g_windowManager.GetWindow(VDM_WINDOW_INFORMATION);
				if (pDialog) pDialog->Close(true);
			}
#endif
...
EVENT_RESULT CGUIWindowFullScreen::OnMouseEvent(const CPoint &point, const CMouseEvent &event)
{
	...
	{ // some other mouse action has occurred - bring up the OSD
...
#if defined(__VIDONME_MEDIACENTER__)
		CGUIDialog *pOSD = NULL;
		if (VidOnMe::RM_VIDONME == VidOnMe::VDMUtils::Instance().GetRunningMode())
		{
			if (event.m_id == ACTION_MOUSE_MOVE && (( point.y > 650 ) || ( point.y < 45 )))
			{
				pOSD = (CGUIDialog*)g_windowManager.GetWindow(VDM_WINDOW_MOVIE_OSD);
			}
			else if (event.m_id == ACTION_MOUSE_LEFT_CLICK && g_settings.m_bMouseClickPause && g_application.IsPlaying())
			{
		...
		{
			pOSD = (CGUIDialogVideoOSD *)g_windowManager.GetWindow(WINDOW_DIALOG_VIDEO_OSD);
		}
#else
...
#if defined(__VIDONME_MEDIACENTER__)
	if (m_timerAudioSwitch.IsRunning() && m_timerAudioSwitch.GetElapsedMilliseconds() > TIME_SWITCH_AUDIO)
	{
		if (g_application.m_pPlayer)
		{
			g_application.m_pPlayer->SetAudioStream(g_settings.m_currentVideoSettings.m_AudioStream);    // Set the audio stream to the one selected
